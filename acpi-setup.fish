    ...
    button/lid)
        case \"\$3\" in
            close)
                ...
                doas -u \$USER gnome-screensaver-command --lock && zzz
                ;;
            open)
                logger \"LID opened\"
                zzz
...
" > /dev/null
    ...
    # After writing the file, you need to make it executable
    doas chmod +x /etc/acpi/handler.sh

    echo "✅ /etc/acpi/handler.sh setup complete."
end
